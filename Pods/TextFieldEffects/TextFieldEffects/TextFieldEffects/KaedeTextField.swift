...
@IBDesignable open class KaedeTextField: TextFieldEffects {
    ...
    override open func animateViewsForTextEntry() {
        ...
        let labelSize = placeholderLabel.intrinsicContentSize.width
        ...
        UIView.animate(withDuration: 0.35, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 2.0, options: .beginFromCurrentState, animations: ({
            self.placeholderLabel.frame.origin = CGPoint(x: self.frame.size.width - labelSize * directionOverride, y: self.placeholderInsets.y)
        }), completion: nil)
        ...
        UIView.animate(withDuration: 0.45, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 1.5, options: .beginFromCurrentState, animations: ({
            self.foregroundView.frame.origin = CGPoint(x: self.frame.size.width - labelSize * directionOverride, y: 0)
        }), completion: { _ in
    ...
    override open func editingRect(forBounds bounds: CGRect) -> CGRect {
        let labelSize = placeholderLabel.intrinsicContentSize.width
        var frame = CGRect(origin: bounds.origin, size: CGSize(width: bounds.size.width - labelSize, height: bounds.size.height))
